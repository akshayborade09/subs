...
import TrialHome, { AdaptiveSheetFrame } from './TrialHome';
...
function FlowGlyph({ icon: Glyph, size = 20, weight = 'regular', tone = 'foreground' }: { icon: Icon; size?: number; weight?: IconWeight; tone?: 'foreground' | 'accent' | 'accentForeground' }) {
  ...
  const color = tone === 'accent' ? (theme === 'dark' ? '#55c986' : '#078a4b') : tone === 'accentForeground' ? (theme === 'dark' ? '#07140c' : '#ffffff') : (theme === 'dark' ? '#ffffff' : '#101010');
...
const initialState: State = { name: '', dob: '', gender: '', food: '', meal: '', dailyMeals: Array.from({ length: 5 }, () => ({ lunch: '', dinner: '' })), bread: '', rice: '', trialDays: [], deliveryLocation: 'B-704, Green View Apartments, Baner Road, Pune 411045', weekendDelivery: 'primary', weekendLocation: '', weekendAddress: '', payment: '', address: emptyAddress(), weekendAddressDetails: emptyAddress() };
const food: Choice[] = [
...
function PaymentButton({ total, enabled, onPress }: { total: number; enabled: boolean; onPress: () => void }) {
  ...
  const dark = theme === 'dark';
  const [width, setWidth] = useState(0);
  return <Pressable accessibilityRole="button" accessibilityState={{ disabled: !enabled }} disabled={!enabled} onPress={onPress} onLayout={(event) => setWidth(event.nativeEvent.layout.width)} style={{ alignSelf: 'stretch', opacity: enabled ? 1 : 0.4 }} className="h-14 flex-row items-center justify-between overflow-hidden rounded-xl px-5">{width > 0 ? <Svg pointerEvents="none" width={width} height={56} preserveAspectRatio="none" style={StyleSheet.absoluteFill}><Defs><LinearGradient id="paymentButtonGradient" x1="0" y1="0" x2="0" y2="1"><Stop offset="0" stopColor={dark ? '#FFFFFF' : '#4D4D4D'} /><Stop offset="1" stopColor={dark ? '#888888' : '#000000'} /></LinearGradient></Defs><Rect width={width} height={56} fill="url(#paymentButtonGradient)" /></Svg> : null}<Text style={{ zIndex: 1 }} className={`font-bold text-base ${dark ? 'text-black' : 'text-white'}`}>Start trial</Text><Text style={{ zIndex: 1 }} className={`font-bold text-base ${dark ? 'text-black' : 'text-white'}`}>Pay ₹{total}</Text></Pressable>;
}
...
function Shell({ title, subtitle, onBack, children, footer, footerDelay = 280, fixedHeader = title === 'Tell us about you', initialScrollOffset = title === 'Your trial, at a glance' ? trialSummaryScrollOffset : 0, onScrollOffsetChange = title === 'Your trial, at a glance' ? (offset) => { trialSummaryScrollOffset = offset; } : undefined, animateContent = title !== 'Your trial, at a glance' }: { title: string; subtitle?: string; onBack?: () => void; children: React.ReactNode; footer?: React.ReactNode; footerDelay?: number; fixedHeader?: boolean; initialScrollOffset?: number; onScrollOffsetChange?: (offset: number) => void; animateContent?: boolean }) {
  ...
  const header = <View style={fixedHeader ? { paddingTop: insets.top + 12 } : undefined} className="bg-canvas px-5 pb-1">
    ...
    <Animated.Text key={`${title}-title`} entering={animateContent ? FadeInUp.delay(30).duration(260) : undefined} className="font-semibold text-[24px] leading-8 tracking-[-0.5px] text-foreground">{title}</Animated.Text>
    {subtitle ? <Animated.Text key={`${title}-description`} entering={animateContent ? FadeInUp.delay(100).duration(260) : undefined} className="mt-2 font-sans text-[15px] leading-6 text-muted">{subtitle}</Animated.Text> : null}
  </View>;
  return (
    ...
    <KeyboardAvoidingView behavior={fixedHeader ? undefined : Platform.OS === 'ios' ? 'padding' : 'height'} className="flex-1 bg-canvas">
      <ScrollView ref={scrollRef} contentOffset={{ x: 0, y: initialScrollOffset }} automaticallyAdjustKeyboardInsets={!fixedHeader} onScroll={(event) => { const offset = event.nativeEvent.contentOffset.y; scrollOffset.current = offset; onScrollOffsetChange?.(offset); }} scrollEventThrottle={16} keyboardDismissMode={Platform.OS === 'ios' ? 'interactive' : 'on-drag'} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false} contentContainerStyle={{ flexGrow: 1, paddingTop: fixedHeader ? 0 : insets.top + 12, paddingBottom: insets.bottom + (footer ? 92 : 24) }}>
        {!fixedHeader ? header : null}
        <Animated.View key={`${title}-content`} entering={animateContent ? FadeInUp.delay(170).duration(280) : undefined} className="mx-5 mt-6">{children}</Animated.View>
      </ScrollView>
    </KeyboardAvoidingView>
      {footer ? <Animated.View key={`${title}-footer`} entering={animateContent ? FadeInUp.delay(footerDelay).duration(280) : undefined} style={{ paddingBottom: insets.bottom + 14 }} className="absolute inset-x-0 bottom-0 bg-canvas px-6 pt-3">{footer}</Animated.View> : null}
    </View>
...
function WheelColumn({ items, selected, onSelect, dayColumn = false }: { items: (string | number)[]; selected: string | number; onSelect: (index: number) => void; dayColumn?: boolean }) {
  ...
  return <FlatList ref={ref} data={items} getItemLayout={(_, index) => ({ length: 44, offset: 44 * index, index })} keyExtractor={(item) => String(item)} className="h-56 flex-1" showsVerticalScrollIndicator={false} snapToInterval={44} decelerationRate="fast" scrollEventThrottle={16} contentContainerStyle={{ paddingVertical: 90 }} onScroll={(event) => selectFromOffset(event.nativeEvent.contentOffset.y)} onMomentumScrollEnd={(event) => selectFromOffset(event.nativeEvent.contentOffset.y)} renderItem={({ item, index }) => <Pressable onPress={() => { onSelect(index); ref.current?.scrollToOffset({ offset: index * 44, animated: true }); }} className="h-11 items-center justify-center"><Text className={`font-semibold text-base ${item === selected ? 'text-white' : 'text-foreground'}`}>{dayColumn ? String(item).padStart(2, '0') : item}</Text></Pressable>} />;
}

function LegacyDateSheet({ value, onClose, onConfirm }: { value: string; onClose: () => void; onConfirm: (value: string) => void }) {
  const now = new Date();
  const saved = value.split(' ');
  const savedDay = Number(saved[0]) || 18;
  const savedMonth = Math.max(0, months.indexOf(saved[1] ?? 'JUL'));
  const savedYear = Number(saved[2]) || Math.min(now.getFullYear(), 1992);
  const [day, setDay] = useState(savedDay); const [month, setMonth] = useState(savedMonth); const [year, setYear] = useState(savedYear);
  const years = Array.from({ length: 100 }, (_, i) => now.getFullYear() - i);
  const valid = new Date(year, month, day).getFullYear() === year && new Date(year, month, day).getMonth() === month && new Date(year, month, day) <= now;
  return <View className="absolute inset-0 z-50 justify-center"><BlurView intensity={Platform.OS === 'android' ? 24 : 32} tint="dark" experimentalBlurMethod={Platform.OS === 'android' ? 'dimezisBlurView' : 'none'} style={StyleSheet.absoluteFill} /><View pointerEvents="none" className="absolute inset-0 bg-black/30" /><Pressable className="absolute inset-0" onPress={onClose} /><Animated.View entering={FadeIn.duration(220)} className="mx-4 rounded-[20px] bg-sheet p-5"><View className="flex-row items-center justify-between gap-3"><Text className="font-semibold text-xl text-foreground">Select date of birth</Text><Pressable accessibilityRole="button" accessibilityLabel="Close date picker" onPress={onClose} className="h-10 w-10 items-center justify-center rounded-full bg-icon-surface"><FlowGlyph icon={XIcon} size={20} weight="bold" /></Pressable></View><View className="relative mt-4 h-56"><View className="absolute inset-x-0 top-[90px] h-11 rounded-xl bg-accent" /><View className="flex-1 flex-row"><WheelColumn items={Array.from({ length: 31 }, (_, i) => i + 1)} selected={day} onSelect={(i) => setDay(i + 1)} dayColumn /><WheelColumn items={months} selected={months[month]!} onSelect={setMonth} /><WheelColumn items={years} selected={year} onSelect={(i) => setYear(years[i]!)} /></View></View>{!valid ? <Text className="mb-3 text-center font-sans text-xs text-red-500">Choose a valid date in the past.</Text> : null}<Primary label="Confirm date" enabled={valid} onPress={() => onConfirm(`${String(day).padStart(2, '0')} ${months[month]} ${year}`)} /></Animated.View></View>;
}
...
function DateSheet({ value, onClose, onConfirm }: { value: string; onClose: () => void; onConfirm: (value: string) => void }) {
  ...
  return <View className="absolute inset-0 z-50 justify-center"><BlurView intensity={Platform.OS === 'android' ? 24 : 32} tint="dark" experimentalBlurMethod={Platform.OS === 'android' ? 'dimezisBlurView' : 'none'} style={StyleSheet.absoluteFill} /><View pointerEvents="none" className="absolute inset-0 bg-black/30" /><Pressable className="absolute inset-0" onPress={onClose} /><Animated.View entering={FadeIn.duration(220)} className="mx-4 rounded-[20px] border border-border bg-sheet p-5"><View className="flex-row items-center justify-between gap-3"><Text className="font-semibold text-xl text-foreground">Select date of birth</Text><Pressable accessibilityRole="button" accessibilityLabel="Close date picker" onPress={onClose} className="h-10 w-10 items-center justify-center rounded-full bg-icon-surface"><FlowGlyph icon={XIcon} size={20} weight="bold" /></Pressable></View><View className="relative mt-4 h-56"><View className="absolute inset-x-0 top-[90px] h-11 rounded-xl bg-accent" /><View className="flex-1 flex-row"><WheelColumn items={Array.from({ length: 31 }, (_, index) => index + 1)} selected={day} onSelect={(index) => setDay(index + 1)} dayColumn /><WheelColumn items={months} selected={months[month]!} onSelect={setMonth} /><WheelColumn items={years} selected={year} onSelect={(index) => setYear(years[index]!)} /></View></View>{!valid ? <Text className="mb-3 text-center font-sans text-xs text-red-500">Choose a valid date in the past.</Text> : null}<Primary label="Confirm date" enabled={valid} onPress={() => onConfirm(`${String(day).padStart(2, '0')} ${months[month]} ${year}`)} /></Animated.View></View>;
}
...
function TrialIntro({ onBack, onProceed, onSkipToSubscribe }: { onBack: () => void; onProceed: () => void; onSkipToSubscribe: () => void }) {
  ...
  return <View className="flex-1 bg-canvas"><ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ flexGrow: 1, paddingTop: insets.top + 12, paddingBottom: insets.bottom + 164 }}><View className="flex-1 px-5"><Pressable accessibilityRole="button" accessibilityLabel="Back" onPress={onBack} className="mb-5 h-11 w-11 items-center justify-center rounded-full border border-border"><FlowGlyph icon={CaretLeftIcon} size={22} weight="bold" /></Pressable><View className="pt-1"><Animated.View entering={FadeInUp.delay(30).duration(260)} className="h-48 items-center justify-center overflow-hidden rounded-[20px] bg-success-soft"><Image source={confirmationMeal} resizeMode="contain" className="h-44 w-44" /></Animated.View><Animated.Text entering={FadeInUp.delay(90).duration(260)} className="mt-7 font-semibold text-[24px] leading-8 tracking-[-0.5px] text-foreground">Let’s start your five-day trial</Animated.Text><Animated.Text entering={FadeInUp.delay(150).duration(260)} className="mt-3 font-sans text-[15px] leading-6 text-muted">Experience fresh home-style meals before choosing a subscription.</Animated.Text><Animated.View entering={FadeInUp.delay(210).duration(260)} className="mt-6 rounded-[16px] bg-surface p-5"><Text className="font-medium text-sm text-accent">TRIAL BENEFIT</Text><Text className="mt-2 font-medium text-lg leading-6 text-foreground">Your five-day trial comes at a discounted price.</Text><Text className="mt-2 font-sans text-[15px] leading-6 text-muted">Choose your food, meals and delivery days next. You can review everything before payment.</Text></Animated.View></View></View></ScrollView><Animated.View entering={FadeInUp.delay(270).duration(260)} style={{ paddingBottom: insets.bottom + 14 }} className="absolute inset-x-0 bottom-0 gap-4 bg-canvas px-6 pt-3"><Primary label={ready ? 'Choose my trial' : 'Preparing your trial…'} enabled={ready} onPress={onProceed} /><Pressable accessibilityRole="button" onPress={onSkipToSubscribe} className="h-12 items-center justify-center rounded-xl border border-border bg-canvas"><Text className="font-semibold text-base text-foreground">Skip to Subscribe</Text></Pressable></Animated.View></View>;
}
...
function createTrialRun(start: Date, weekendDelivery: WeekendDelivery, skippedWeekendKeys: Set<string> = new Set()) {
  ...
  let daysInWindow = 0;
  while (selected.length < 5 && daysInWindow < 7) {
...
function LegacyWindowCalendarSheet({ initialDays, initialWeekendDelivery, initialWeekendLocation, initialWeekendAddress, onClose, onConfirm }: { initialDays: string[]; initialWeekendDelivery: WeekendDelivery; initialWeekendLocation: string; initialWeekendAddress: string; onClose: () => void; onConfirm: (days: string[], weekendDelivery: WeekendDelivery, weekendLocation: string, weekendAddress: string) => void }) {
  ...
  const calendarDays = Array.from({ length: 9 }, (_, index) => addCalendarDays(today, index - 2));
  const savedDays = initialDays.filter((key) => calendarDays.some((date) => dateKey(date) === key)).slice(0, 5);
...
function TrialCalendarSheet({ initialDays, initialWeekendDelivery, initialWeekendLocation, initialWeekendAddress, onClose, onConfirm }: { initialDays: string[]; initialWeekendDelivery: WeekendDelivery; initialWeekendLocation: string; initialWeekendAddress: string; onClose: () => void; onConfirm: (days: string[], weekendDelivery: WeekendDelivery, weekendLocation: string, weekendAddress: string) => void }) {
  ...
  const [selectedKeys, setSelectedKeys] = useState<Set<string>>(new Set(initialDays.slice(0, 5)));
  ...
  const isContinuous = selectedDates.length === 5 && selectedDates.every((date, index) => {
  ...
  return <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} className="absolute inset-0 z-50 justify-end">
    ...
    <AdaptiveSheetFrame onClose={onClose} title="Choose your five days" onExpansionChange={setCalendarExpanded}>{(sheetControls) =>
      <ScrollView
      ...
      >
        <View className="px-5">
          <Text className="font-sans text-[15px] leading-6 text-muted">Choose five continuous delivery days. Saturday and Sunday may be skipped.</Text>
        </View>
        <Animated.ScrollView
        ...
        <View className="mx-5 mt-5 rounded-[16px] bg-surface p-4">
          <View className="flex-row items-center justify-between">
            ...
            <Text className={`font-semibold text-sm ${isContinuous ? 'text-accent' : 'text-muted'}`}>{selectedDates.length}/5 selected</Text>
          </View>
          ...
          {selectedDates.length === 5 && !isContinuous ? <Text className="mt-2 font-medium text-sm text-red-500">Keep weekdays continuous; only Saturday and Sunday can be skipped.</Text> : null}
        </View>
        <View className="mx-5 mt-6">
          <Primary label="Confirm five days" enabled={isContinuous} onPress={() => onConfirm(selectedDates.map(dateKey), initialWeekendDelivery, initialWeekendLocation, initialWeekendAddress)} />
        </View>
      </ScrollView>}
    </AdaptiveSheetFrame>
  </KeyboardAvoidingView>;
...
export default function TrialFlow() {
  ...
  const meals = data.meal === 'Both' ? 10 : 5;
  ...
  if (step === 'personal') return <><Shell title="Tell us about you" subtitle="A few details help us personalise your trial." onBack={undefined} footer={<Primary label="Continue" enabled={data.name.trim().length > 1 && !!data.dob && !!data.gender} onPress={next} />}><View className="gap-5"><Field animationDelay={190} autoFocus label="Full name" value={data.name} onChangeText={(v) => set('name', v)} placeholder="Your full name" onSubmitEditing={() => { Keyboard.dismiss(); setTimeout(() => setDateOpen(true), 120); }} /><Animated.View entering={FadeInUp.delay(260).duration(240)}><Text className="mb-2 font-medium text-sm tracking-[0.4px] text-muted">DATE OF BIRTH</Text><Pressable accessibilityRole="button" onPress={() => { Keyboard.dismiss(); setTimeout(() => setDateOpen(true), 120); }} className="h-14 justify-center rounded-xl border border-surface bg-surface px-4"><Text className={`font-sans text-lg font-semibold leading-6 ${data.dob ? 'text-foreground' : 'text-muted'}`}>{data.dob || 'DD MMM YYYY'}</Text></Pressable></Animated.View><Animated.View entering={FadeInUp.delay(330).duration(240)}><Text className="mb-2 font-medium text-sm tracking-[0.4px] text-muted">GENDER</Text><View className="flex-row gap-2">{['Woman', 'Man', 'Non-binary'].map((gender) => { const selected = data.gender === gender; return <Pressable key={gender} accessibilityRole="radio" accessibilityState={{ checked: selected }} onPress={() => set('gender', gender)} className={`h-12 flex-1 items-center justify-center rounded-xl border ${selected ? 'border-[3px] border-accent bg-accent/10' : 'border-surface bg-surface'}`}><Text className="font-semibold text-base text-foreground">{gender}</Text></Pressable>; })}</View></Animated.View></View></Shell>{dateOpen ? <DateSheet value={data.dob} onClose={() => setDateOpen(false)} onConfirm={(v) => { set('dob', v); setDateOpen(false); }} /> : null}</>;
