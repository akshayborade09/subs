...
function Glyph({ icon: GlyphIcon, color }: { icon: Icon; color?: string }) {
  ...
  return <GlyphIcon size={18} weight="bold" color={color ?? (theme === 'dark' ? '#ffffff' : '#101010')} />;
}
...
function IconText({ icon, title, description, color, trailing, titleClassName = 'font-semibold text-base text-foreground' }: { icon: Icon; title: string; description?: string; color?: string; trailing?: ReactNode; titleClassName?: string }) {
  return <View className="flex-row items-start gap-2"><View className="h-6 w-6 items-center justify-center"><Glyph icon={icon} color={color} /></View><View className="flex-1"><Text className={`${titleClassName} leading-6`}>{title}</Text>{description ? <Text className="mt-1 font-sans text-sm leading-5 text-muted">{description}</Text> : null}</View>{trailing ? <View className="h-6 items-center justify-center">{trailing}</View> : null}</View>;
}
...
function GreenButton({ label, onPress }: { label: string; onPress: () => void }) {
  return <Pressable accessibilityRole="button" onPress={onPress} className="min-h-14 items-center justify-center rounded-xl bg-[#064e3b] px-5"><Text className="font-bold text-base text-white">{label}</Text></Pressable>;
}
...
function RewardPage({ onBack, go }: { onBack: () => void; go: (route: Route) => void }) {
  return <View className="flex-1"><Header onBack={onBack} eyebrow="REWARD EARNED" title="Your free meal day is ready" description="You completed one qualifying paid month. Choose an eligible delivery day within 60 days." /><View className="mt-10 items-center"><View className="h-24 w-24 items-center justify-center rounded-full bg-success-soft"><Glyph icon={GiftIcon} color="#078a4b" /></View></View><Section><Card><MetaRow label="Reward" value="One free lunch & dinner day" /><MetaRow label="Earned" value="22 July 2026" /><MetaRow label="Use by" value="20 September 2026" /><MetaRow label="Value" value="Applied automatically" /></Card></Section><View className="mt-auto pt-6"><PrimaryButton label="Choose free meal day" onPress={() => go('redeem')} /></View></View>;
}
...
function RedeemPage({ onBack, toast, go }: { onBack: () => void; toast: (message: string) => void; go: (route: Route) => void }) {
  ...
  return <><Header onBack={onBack} title="Choose your free meal day" description="Select an eligible date. Your current meal and address settings will be used." /><Section title="Eligible dates"><View className="flex-row justify-between gap-2">{dates.map((date, index) => <Pressable key={date} onPress={() => setSelected(index)} className={`min-h-16 flex-1 items-center justify-center rounded-xl border ${selected === index ? 'border-[3px] border-accent bg-success-soft' : 'border-field bg-field'}`}><Text className={`text-center font-semibold text-sm ${selected === index ? 'text-accent' : 'text-foreground'}`}>{date}</Text></Pressable>)}</View></Section><Section title="Reward details"><Card tone="success"><MetaRow label="Meal" value="Lunch & dinner" /><MetaRow label="Food" value="Mix of both" /><MetaRow label="Payable" value="₹0" valueTone="text-accent" /></Card></Section><Section title="Delivery address"><Card><IconText icon={MapPinIcon} color="#078a4b" title="Home" description="B-704, Green View Apartments, Baner Road, Pune 411045" /></Card></Section><View className="mt-6"><PrimaryButton label="Confirm free meal" onPress={() => { toast(`Free meal scheduled for ${dates[selected]}`); go('loyalty'); }} /></View></>;
}
