...
export default function TrialHome({ food, meal, dailyMeals = [], bread, rice, address, lunchDelivery = null, dinnerDelivery = null, openSubscriptionOnLoad = false, openMealDetailOnLoad = false, lifecycleVariant = 'trial_active', initialPlanResumeDateKey = null, bottomInset = 0, onPaymentStatusPress, onProfilePress, onExploreMyPlanPress }: { food: string; meal: string; dailyMeals?: Array<{ lunch: string; dinner: string }>; bread: string; rice: string; address: string; lunchDelivery?: TrialMealDeliveryState | null; dinnerDelivery?: TrialMealDeliveryState | null; openSubscriptionOnLoad?: boolean; openMealDetailOnLoad?: boolean; lifecycleVariant?: HomeLifecycleVariant; initialPlanResumeDateKey?: string | null; /** Extra scroll padding so content clears the floating subscriber nav. */ bottomInset?: number; onPaymentStatusPress?: () => void; onProfilePress?: () => void; onExploreMyPlanPress?: () => void }) {
  const insets = useSafeAreaInsets();
  const { theme } = useUniwind();
  const dark = theme === 'dark';
  const seed = useMemo(() => {
  ...
  return <View className="flex-1 bg-canvas"><ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingTop: insets.top + 20, paddingBottom: insets.bottom + 32 + bottomInset }}><View className="px-5">
