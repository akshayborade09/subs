...
const initialMeals = (food: string, bread: string, rice: string, meal: string, address: string, dailyMeals: Array<{ lunch: string; dinner: string }> = []): TrialMeal[] => [
  ...
  { id: '24', date: 'Thursday, 24 July', dayLabel: 'THU', shortDate: '24', mealType: 'Dinner', status: 'upcoming', foodPreference: food, breadPreference: bread, ricePreference: rice, addressLabel: 'Home', address, nutrition },
...
function UpcomingRipple({ color = 'green' }: { color?: 'green' | 'red' }) {
  ...
  return <NativeAnimated.View pointerEvents="none" style={{ opacity, transform: [{ scale }], borderColor: color === 'red' ? '#dc2626' : '#078a4b', backgroundColor: color === 'red' ? '#fee2e2' : '#e7f7ee' }} className="absolute h-7 w-7 rounded-full border-2" />;
}
...
function TrialDayTracker({ meals, selectedId, showBoth, onSelectDate, onOpenMeal }: { meals: TrialMeal[]; selectedId: string; showBoth: boolean; onSelectDate: (meal: TrialMeal) => void; onOpenMeal: (meal: TrialMeal) => void }) {
  return <View className="w-full flex-row">{meals.map((meal) => { const excluded = meal.isPlanDay === false; const selected = meal.id === selectedId && !excluded; const markers = meal.mealMarkers?.slice(0, showBoth ? 2 : 1) ?? Array.from({ length: showBoth ? 2 : 1 }, () => ({ foodPreference: meal.foodPreference, status: meal.status })); return <View key={meal.id} className="flex-1 items-center"><Pressable disabled={excluded} accessibilityRole="button" accessibilityLabel={excluded ? `${meal.date}, no meal selected` : `Select ${meal.date}`} accessibilityState={{ selected, disabled: excluded }} onPress={() => onSelectDate(meal)} className={`h-14 w-full max-w-[46px] items-center justify-center rounded-[14px] border ${excluded ? 'border-surface bg-surface opacity-45' : selected ? 'border-accent/10 bg-accent/10' : 'border-surface bg-surface'}`}><Text className={`font-bold text-lg ${excluded ? 'text-muted' : selected ? 'text-accent' : 'text-foreground'}`}>{meal.shortDate}</Text><Text className="mt-0.5 font-medium text-[11px] text-muted">{meal.dayLabel}</Text></Pressable><View className="mt-2 items-center gap-1">{markers.map((marker, markerIndex) => { const nonVeg = marker.foodPreference.toLowerCase().includes('non'); const active = selected && marker.status === 'upcoming'; const delayed = marker.status === 'delayed'; const failed = marker.status === 'delivery_failed' || marker.status === 'issue'; const delivered = marker.status === 'delivered'; const borderColor = excluded || marker.status === 'inactive' || marker.status === 'paused' ? '#d8d8d8' : failed ? '#dc2626' : delayed ? '#f59e0b' : nonVeg ? '#dc2626' : '#078a4b'; return <Pressable key={`${meal.id}-${markerIndex}`} disabled={excluded} accessibilityRole="button" accessibilityLabel={`${markerIndex === 0 ? 'Lunch' : 'Dinner'}, ${marker.foodPreference}, ${marker.status}`} onPress={() => onOpenMeal(meal)} className={`h-7 w-9 items-center justify-center ${excluded ? 'opacity-45' : ''}`}>{active ? <UpcomingRipple color={nonVeg ? 'red' : 'green'} /> : null}<View style={{ borderColor, backgroundColor: delivered ? borderColor : 'transparent' }} className="h-5 w-5 items-center justify-center rounded-full border-[3px]">{delivered && !excluded ? <HomeGlyph icon={CheckIcon} size={16} weight="bold" tone="white" /> : null}</View></Pressable>; })}</View></View>; })}</View>;
}
...
export default function TrialHome({ food, meal, dailyMeals = [], bread, rice, address, openSubscriptionOnLoad = false, lifecycleVariant = 'trial_active' }: { food: string; meal: string; dailyMeals?: Array<{ lunch: string; dinner: string }>; bread: string; rice: string; address: string; openSubscriptionOnLoad?: boolean; lifecycleVariant?: HomeLifecycleVariant }) {
  const insets = useSafeAreaInsets();
  const { theme } = useUniwind();
  const dark = theme === 'dark';
  const seed = useMemo(() => {
    if (lifecycleVariant.startsWith('subscription_')) {
      ...
      if (lifecycleVariant === 'subscription_delivery_delayed') return week.map((item, index) => index === 2 ? { ...item, status: 'delayed' as MealStatus, mealMarkers: item.mealMarkers?.map((marker, markerIndex) => markerIndex === 0 ? { ...marker, status: 'delayed' as MealStatus } : marker) } : item);
      if (lifecycleVariant === 'subscription_delivery_failed') return week.map((item, index) => index === 2 ? { ...item, status: 'delivery_failed' as MealStatus, mealMarkers: item.mealMarkers?.map((marker, markerIndex) => markerIndex === 0 ? { ...marker, status: 'delivery_failed' as MealStatus } : marker) } : item);
    ...
    if (lifecycleVariant === 'trial_scheduled') return trial.map((item) => ({ ...item, status: 'upcoming' as MealStatus, items: undefined }));
  ...
  const configs: Record<HomeLifecycleVariant, { eyebrow: string; title: string; description: string; caption?: string; selectedLabel: string }> = {
    trial_scheduled: { eyebrow: 'TRIAL SCHEDULED', title: 'Your trial starts soon', description: 'Your five selected trial dates are ready. Tap a meal-status circle to review details.', caption: 'Trial starts 27 July', selectedLabel: 'FIRST TRIAL MEAL' },
    trial_active: { eyebrow: 'ACTIVE TRIAL', title: 'Your five-day trial', description: 'Tap a meal-status circle to view that meal’s details.', selectedLabel: 'SELECTED MEAL' },
    trial_subscription_purchased: { eyebrow: 'ACTIVE TRIAL', title: 'Your five-day trial', description: 'Your subscription is ready and will begin after the final trial meal.', caption: 'Subscription starts after trial', selectedLabel: 'SELECTED MEAL' },
    trial_completed: { eyebrow: 'TRIAL COMPLETE', title: 'Your five-day trial is complete', description: 'Review your delivered meals or continue with a subscription.', caption: 'Trial completed', selectedLabel: 'LAST TRIAL MEAL' },
  ...
  const planCard = lifecycleVariant === 'subscription_expired'
    ...
    : lifecycleVariant === 'subscription_renewal_failed'
      ...
      : lifecycleVariant === 'subscription_paused'
        ...
        : undefined;
  const stateNotice = lifecycleVariant === 'subscription_delivery_delayed'
  ...
  return <View className="flex-1 bg-canvas">{activeTab === 'home' ? <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingTop: insets.top + 20, paddingBottom: insets.bottom + 32 }}><View className="px-5">
    ...
    {stateNotice ? <Animated.View entering={FadeInUp.delay(190).duration(260)} style={{ backgroundColor: stateNotice.tone === 'orange' ? (dark ? '#2A1B0C' : '#FFF3E0') : stateNotice.tone === 'red' ? (dark ? '#2B1212' : '#FDECEC') : stateNotice.tone === 'blue' ? (dark ? '#101F33' : '#EAF3FF') : (dark ? '#211A2E' : '#F2ECFF'), borderColor: stateNotice.tone === 'orange' ? (dark ? '#6A4318' : '#F2C98F') : stateNotice.tone === 'red' ? (dark ? '#663030' : '#F2B7B7') : stateNotice.tone === 'blue' ? (dark ? '#29486E' : '#BCD8F8') : (dark ? '#4A3867' : '#D9C8FF') }} className="mt-5 rounded-[16px] border p-4"><Text className="font-semibold text-base text-foreground">{stateNotice.title}</Text><Text className="mt-1 font-sans text-[14px] leading-5 text-muted">{stateNotice.body}</Text>{stateNotice.action ? <View className="mt-4"><Primary label={stateNotice.action} onPress={() => showToast('Re-subscription selected')} /></View> : null}</Animated.View> : null}
    <Animated.View entering={FadeInUp.delay(210).duration(280)} className="mt-6"><TrialDayTracker meals={meals} selectedId={selectedId} showBoth={(subscription?.meal ?? meal) === 'Both'} onSelectDate={() => {}} onOpenMeal={(item) => { setDetailId(item.id); setSheetOpen(true); }} /></Animated.View>
