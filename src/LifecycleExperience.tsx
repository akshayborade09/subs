...
function RecoveryState({ id }: { id: LifecycleStateId }) {
  ...
  return <><View className="mt-7 rounded-[16px] border border-border bg-sheet p-5"><View className="h-11 w-11 items-center justify-center rounded-full bg-icon-surface"><Glyph icon={config.icon} color={config.color} /></View><Text className="mt-5 font-semibold text-xl text-foreground">{config.title}</Text><Text className={`mt-2 ${headingDescriptionClass}`}>{config.body}</Text>{id !== 'S' ? <View className="mt-5 gap-3"><MetaRow label="Affected meal" value="Lunch · 23 July" /><MetaRow label="Current address" value="Home · Baner" /><MetaRow label="Credit status" value={id === 'R' ? 'Pending review' : 'Not applicable'} /></View> : null}</View>{id !== 'S' ? <View className="mt-4 rounded-[16px] border border-border bg-sheet p-5"><Text className="font-semibold text-lg text-foreground">Upcoming deliveries</Text><WeekTracker /></View> : null}</>;
}
